"""Executable placement policies for iDynamics Policy 1-4.

In IEEE TSC paper, the policies are described as follows:

Policy 1 is renamed as CGA (Call-graph–Aware) Policy;
Policy 4 is renamed as HDA (Hybrid-Dynamics–Aware) Policy.
"""
...
class BasePlacementPolicy:
    """Greedy capacity-aware planner shared by all paper-facing policies."""
...
class Policy1TrafficAffinity(BasePlacementPolicy):
    """Policy 1: co-locate high-stress call-graph edges."""
...
class Policy2LatencyCriticalPath(BasePlacementPolicy):
    """Policy 2: optimize low-delay paths for latency-critical edges."""
...
class Policy3BandwidthPayloadAware(BasePlacementPolicy):
    """Policy 3: optimize high-payload flows against available bandwidth."""
...
class Policy4HybridDynamics(BasePlacementPolicy):
    """Policy 4: combine traffic stress, latency, and bandwidth."""
