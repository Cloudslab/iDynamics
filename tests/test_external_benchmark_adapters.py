...
def test_adapter_apply_and_delete_commands_are_reversible() -> None:
    ...
    assert apply_commands == ("kubectl apply -n idyn-general -f external/benchmarks/deathstarbench/hotelReservation/kubernetes",)
    assert delete_commands == ("kubectl delete -n idyn-general -f external/benchmarks/deathstarbench/hotelReservation/kubernetes --ignore-not-found",)
