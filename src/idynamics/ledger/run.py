"""Central run-ledger utility for reproducible experiments."""
...
def init_run_ledger(
    ...
    (run_root / "environment.txt").write_text(f"python: {sys.version.split()[0]}\n")
    (run_root / "commands.log").touch()
    if not (run_root / "summary.md").exists():
        (run_root / "summary.md").write_text(f"# {run_id}\n\nStatus: initialized\n\n## Purpose\n{purpose}\n")
    if not (run_root / "paper_claims.md").exists():
        (run_root / "paper_claims.md").write_text("# Paper Claims\n\n- No paper claims have been made from this run yet.\n")
    ...
    ledger = ExperimentLedger(
        ...
        environment_path=run_root / "environment.txt",
        summary_path=run_root / "summary.md",
        paper_claims_path=run_root / "paper_claims.md",
    )
